                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    // ground plane.
    Vector3 y_vector = Vector3(0, 1, 0);
    Point2 start2d = stroke2d[0];
    Point2 end2d = stroke2d[stroke2d.size()-1];
    Point3 start3d;
    Point3 end3d;
    ScreenPtHitsGround(view_matrix, proj_matrix, start2d, &start3d);
    ScreenPtHitsGround(view_matrix, proj_matrix, end2d, &end3d);


    Vector3 plane = start3d-end3d;
    Vector3 normal = (y_vector.Cross(plane)).ToUnit();
    ...
    for (int i = 0; i < stroke2d.size(); i++) {
        Point3 in_3d = GfxMath::ScreenToNearPlane(view_matrix, proj_matrix, stroke2d[i]);
        Vector3 direction = (in_3d - camera_pos).ToUnit();
        Ray ray(camera_pos, direction);

        float hit_time;
        Point3 hit_point;

       // ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[i], &temp);
        bool check = ray.IntersectPlane(start3d, normal, &hit_time, &hit_point);
        s_curve.push_back(hit_point);
    }
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
        Point3 P = ground_mesh_.vertexReadOnly(i); // original vertex

        // adjust P according to equations...
        Point3 closest_point = P.ClosestPointOnPlane(start3d,normal);
        float h = hfunc(normal, s_curve, closest_point);
        if (h!=0) {
           float d = P.DistanceToPlane(start3d, normal);
           float w = fmax(0.0, (1.0 - pow(d/5.0,2)));
           P[1] = (1.0 - w) * P[1] + w * h;
        }
