...
void EdgeMesh::CreateFromMesh(const Mesh &mesh) {
    ...
    for (int t = 0; t < mesh.num_triangles(); t++) {
        std::vector<unsigned int> tri = mesh.triangle_verticesReadOnly(t);
        Point3 a = mesh.vertexReadOnly(tri[0]);
        Point3 b = mesh.vertexReadOnly(tri[1]);
        Point3 c = mesh.vertexReadOnly(tri[2]);
        Vector3 e1 = b-a;
        e1.Normalize();
        Vector3 e2 = c-a;
        e2.Normalize();
                       ...
                       const Mesh &mesh, int v0, int v1, Vector3 n)
{
    ...
    if (it != edgeMap.end()) { // found
        ...
        vertices->push_back(mesh.vertexReadOnly(v0));
        vertices->push_back(mesh.vertexReadOnly(v0));
        vertices->push_back(mesh.vertexReadOnly(v1));
        vertices->push_back(mesh.vertexReadOnly(v1));
        
        normals->push_back(Vector3());
        normals->push_back(mesh.normalReadOnly(v0));
        normals->push_back(Vector3());
        normals->push_back(mesh.normalReadOnly(v1));
